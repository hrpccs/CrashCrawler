...
2. 在进程资源被回收前把进程的重要信息读取（与进程相关的由eBPF读取）
   ...
   - backtrace（已完成）：目前能够读取到崩溃进程的内核函数调用栈，并且通过内核符号表kallsyms得到对应的内核函数名与偏移量。
   
   - 引发崩溃的信号（已完成）：可以读取到进程执行do_exit（）内核函数时传入参数**exitcode**，在 进程异常退出时读取 exitcode的低7位，可以读到引发崩溃的信号。
   
   - 软件包版本与软件包依赖（基本完成）：目前可以在eBPF程序中通过模拟内核函数**d_path**的实现（用于读取出动态库的路径），能够读取到进程依赖的动态库依赖，但是无法获取该进程的一些静态编译的依赖（这些应该只有包管理器能够清楚，或者通过编译文件Makefile等，但是从我们崩溃收集组件的角度来说几乎无法获取）。
   
   - 硬件架构与操作系统版本信息（已完成）：通过**lscpu**命令在我们的崩溃收集组件部署阶段就可以从部署机读取出来，这属于静态信息。

3. 分析收集到的崩溃信息，生成崩溃报告（初步完成）
   
   组件的用户态程序接收到eBPF从内核回传到用户态的崩溃进程信息后，可以把崩溃信息持久化保存在特定的路径。这部分信息主要是帮助开发者快速获取到引发该进程崩溃的信号，与相关的虚拟内存映射信息。
   
   进一步目标：收集所有的崩溃进程内存信息，比如堆栈段的内存使用等，有利于定位因内存引发的进程崩溃。
...
我们需要在任何进程发生崩溃的时候，能够在操作系统把进程资源回收之前，把进程的与崩溃有关的信息保存下来。这就对我们的崩溃进程拦截手段有个硬性要求——必须是触发式地拦截。
...
**结论**：**core dump机制**并不适合拦截整个操作系统层面的所有的进程是否发生崩溃并获取信息，它更适合某个用户在debug某个特定程序时提供内存镜像来协助调试工具进行debug。
...
sched_process_exit在do_exit()内核源码里面插入的地方在exit_mm函数（释放进程占有内存的函数）的后面，所以没办法读取进程占有内存里面的一些信息。
...
以上就是崩溃进程收集模块的设计思路了，具体实现主要是eBPF程序，具体实现可以看`exitcatch.c` 的主函数和事件处理函数和`exitcatch.bpf.c`里的BPF程序。
...
软件初始化完成后，我们将收集非正常退出进程的信息，如：崩溃时间，崩溃指令，崩溃进程信号等基本信息，以及崩溃进程的**栈追踪**和线程的**动态依赖库**。在组织文件输出格式是，我们大量参考了Linux中信息展现形式，如动态依赖库信息展示格式类似于`/proc/<pid>/maps`的格式与内容，旨在让使用者快速熟悉信息内容。
...
3. 把崩溃报告用可视化的形式展示出来，更方便开发者查看。

4. 利用数据库存储进程崩溃信息，崩溃日志的管理更加方便，高效。

5. 未来可以利用libbpf + CO-RE 进行多版本可移植分发。
