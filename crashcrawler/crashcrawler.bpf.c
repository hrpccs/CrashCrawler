...
static long process_vma(u32 index,void* ctx){
    struct loop_ctx* lctx = ctx;
    struct vm_area_struct* vma = lctx->vma;
    struct event* e = lctx->e;
    if(vma == NULL){
        return 1;
    }
    struct file* file = BPF_CORE_READ(vma, vm_file);
    if (!file) {
        lctx->vma = BPF_CORE_READ(vma,vm_next);
        return 0;
    }
    ...
    e->mmap[lctx->count].dev = BPF_CORE_READ(f_inode, i_sb, s_dev);
    e->mmap[lctx->count].ino = BPF_CORE_READ(f_inode, i_ino);
    e->mmap[lctx->count].pgoff = BPF_CORE_READ(vma, vm_pgoff) << PAGE_SHIFT;
    e->mmap[lctx->count].start = BPF_CORE_READ(vma, vm_start);
    e->mmap[lctx->count].end = BPF_CORE_READ(vma, vm_end);
    e->mmap[lctx->count].flags = BPF_CORE_READ(vma, vm_flags);
    lctx->count++;
    lctx->vma = BPF_CORE_READ(vma,vm_next);
    return 0;
}
// bpf_loop
SEC("kprobe/do_exit")
int BPF_KPROBE(kprobe__do_exit_no_path_bpf_loop, long exitcode) {
    ...
    if (e) {
       ...
       bpf_loop((1<<22),process_vma,&lctx,0);
        e->count = lctx.count;
